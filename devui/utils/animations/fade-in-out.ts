...
export const fadeInOut: AnimationTriggerMetadata = trigger('fadeInOut', [
  state('void', style({transform: 'scaleY(0.8) translateY(-4px)', opacity: 0.8, transformOrigin: '0 0%', display: 'block'})),
  ...
  transition('bottom => void', [
    animate(`200ms ${easeInQuint}`,
      style({transform: 'scaleY(0.8) translateY(-4px)', opacity: 0.8, transformOrigin: '0 0%', display: 'none'})),
  ]),
  transition('void => top', [
  ...
  transition('top => void', [
    ...
    style({transform: 'scaleY(0.8) translateY(4px)', opacity: 0.8, transformOrigin: '0 100%', display: 'none'}))
  ])]);
