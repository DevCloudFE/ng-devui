...
| mode |  [`TimeAxisMode`](#timeaxismode)   |  `'normal'`  | 可选，`normal`默认排布， `alternative`交替排布,  `top` \| `bottom` \| `left` \| `right`内容按指定位置单边排布    | [时间节点内容方向自定义](demo#content-with-alternative-mode)  |
...
|  position  | `'top'\|'bottom'\|'left'\|'right'` |  --  | 可选，text 或 data的位置，若有time则time位于相反位置   | [时间节点内容方向自定义](demo#content-with-alternative-mode) |
...
| mode |  [`TimeAxisMode`](#timeaxismode)   |  `'normal'`  | 可选，`normal`默认排布， `alternative`交替排布,  `top` \| `bottom` \| `left` \| `right`内容按指定位置单边排布      | [时间节点单独使用](demo#seperate-way)  |
...
|  position  | `'top'\|'bottom'\|'left'\|'right'` |  --  | 可选，text 或 data的位置，若有time则time位于相反位置   | [时间节点单独使用](demo#seperate-way) |
