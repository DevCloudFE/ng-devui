...
|     beforeTagChange     |        `function`        |   --    | Optional. Method called before changing the tag, returns the boolean type, and returns false to prevent the classification value from changing.                                   |                                                           |
|       toggleEvent       |        `function`        |   --    | Optional. Method called when the drop-down menu switch of the selected classification is enabled, which can be executed by the default method after being blocked by return.      |              [Basic usage](demo#basic-usage)              |
