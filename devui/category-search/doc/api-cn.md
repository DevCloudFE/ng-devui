...
|     beforeTagChange     |        `function`        |  --   | 可选，改变标签前调用的方法，返回 boolean 类型，返回 false 可以阻止分类值改变                |                                      |
|       toggleEvent       |        `function`        |  --   | 可选，已选分类的下拉菜单开关时调用的方法，可使用 return 阻止之后的默认方法执行              | [基本用法](demo#basic-usage)         |
