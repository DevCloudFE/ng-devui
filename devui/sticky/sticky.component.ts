...
export class StickyComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    ngAfterViewInit() {
        ...
        if (this.scrollTarget !== window) {
          this.subscription = fromEvent<Event>(window, 'scroll').pipe(
            throttleTime(100, undefined, {leading: true, trailing: true}),
            filter(event =>
              event.target !== this.scrollTarget && (
                event.target === window
                || event.target === document  // fix ie11 document.contains is not defined
                || (<HTMLElement>event.target).contains
                  && (<HTMLElement>event.target).contains(this.scrollTarget)
               )
            )
          ).subscribe(event => {
            this.statusProcess(this._status);
    ...
    scrollHandler = () => {
       ...
       if (this.parentNode.getBoundingClientRect().top - viewOffsetTop > (this.view && this.view.top || 0)) {
           ...
           - parseInt(computedStyle.borderBottomWidth, 10)
               < viewOffsetTop + (this.view && this.view.top || 0)
