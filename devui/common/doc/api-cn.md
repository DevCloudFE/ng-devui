...
| target | `string` | '\_black' | 可选，指定跳转超链接的 target, 类型为[['_blank' \| '_self' \| '_parent' \| '_top' \| string]] | [Helper Utils](demo#helper-utils) |
...
|      option       |              `Object`               |                 --                  |                                                                                       可选                                                                                        |
|   option.method   | `'POST'\| 'GET' \| 'post' \| 'get'` |               'post'                |                                                                  可选，类型为['POST'\| 'GET' \| 'post'\| 'get']                                                                   | [Helper Utils](demo#helper-utils) |
|   option.params   |              `Object`               |                 --                  | 可选，参数对象左值类型为 string，右值类型为 string。对应表单的表单域,method 类型为 GET/get 的时候，会将参数拼接到 url 上，method 类型为 POST/post 时候，参数会拼接才 payload 里。 | [Helper Utils](demo#helper-utils) |
...
| 参数  | 类型   | 默认    | 说明                                              | 跳转 Demo                                                              |
| ----- | ------ | ------- | ------------------------------------------------- | ---------------------------------------------------------------------- |
| event | string | 'click' | 可选，值为 MouseEvent 的 type，指定冒泡的鼠标事件 | [Iframe Event Propagate API](demo#iframe-propagate) |
...
|  position  | `'top'\|'right'\|'bottom'\|'left' or ('top'\|'right'\|'bottom'\|'left')[]` | 'top' | 可选，复制成功提示弹出方向，如果传入数组形式，则当前将按照传入数组次序，自适应选取一个方向 | [复制到剪贴板指令](demo#clipboard) |
...
|      事件       |        类型         |                                         说明                                         | 跳转 Demo                                             |
| :-------------: | :-----------------: | :----------------------------------------------------------------------------------: | ----------------------------------------------------- |
| copyResultEvent | `EventEmitter<any>` | 复制后发出的事件，如果是不支持复制操作，isSupported 为 false，可用于提示用户自行复制 | [复制到剪贴板指令](demo#clipboard) |
