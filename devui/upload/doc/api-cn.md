...
|      alertMsgEvent      | `EventEmitter<Message[]>` | 上传文件不符合上传文件配置uploadOptions时的回调函数，返回错误提示信息息                                                                         | [任意区域上传](demo#customize-area-upload) |
...
`fileUploaders`：FileUploader 类型的数组，FileUploader中包含 send()， cancel()方法
